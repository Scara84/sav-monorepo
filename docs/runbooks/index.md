...
| [cutover.md §V1.6](cutover.md#v16--backfill-sav_filesonedrive_item_id-pattern-b-opération-manuelle) — Backfill `sav_files.onedrive_item_id` (V1.6 — 2026-05-08) | Tech-lead | 6 lignes pré-cutover avec `onedrive_item_id` pollué (filename au lieu de Graph ID). Procédure manuelle 6 UPDATE one-by-one + CSV backup UTF-8 + audit_trail INSERTs + synthèse session. |
