...
- **Dépendance Make.com** : actuellement le webhook sort un email. Si Make.com tombe, pas d'email. **Mitigation** : retry queue persistée en DB (tâches d'envoi reprises au déblocage Make.com) + canal SMTP serveur-à-serveur en fallback (provider à choisir au PRD — pas Infomaniak qui a été sorti du périmètre en Epic 1).
