...
### Review Findings — 2026-04-28 (second pass adversarial 3 layers)

CR adversarial 3 couches relancé après le commit `4273047` (CR initial). 56 findings bruts triés → 14 patches **appliqués** + 3 defers (W53-W55 dans `deferred-work.md`).

**Validation post-patches :**
- Tests : 866/866 (vs baseline 865 ; +1 test EMPTY_RESULT P13).
- Typecheck : 0.
- Lint:business : 0.
- Build : 460.44 KB ≡ baseline (pas de drift bundle).

Détails patches infra (P1→P14, tous cochés `[x]`).

- [x] [Review][Patch] **P1 (HIGH) — CSV injection : `escapeCsvCell` ne neutralise pas `=`/`+`/`-`/`@`/`\t` en début de cellule** [`client/api/_lib/reports/csv-generator.ts:escapeCsvCell`] — fields user-controllables (member.first_name/last_name, member.email, group.name, tags[], motifs, supplier_code, invoice_ref) flow unchanged ; un membre nommé `=cmd|'/c calc'!A1` exécute la formule à l'ouverture Excel/LibreOffice. Fix : prepend `'` sentinel + force-quote quand `^[=+\-@\t\r]/.test(str)`.
- [x] [Review][Patch] **P2 (MEDIUM) — `formatEurFr` produit `'123,45.67'` malformé pour cents non-entiers** [`client/api/_lib/reports/csv-generator.ts:formatEurFr`] — `Number.isFinite` est le seul guard ; un upstream qui passe `12345.67` génère `'123,45.67'`. Fix : `Math.round(cents)` avant split euros/remainder.
- [x] [Review][Patch] **P3 (MEDIUM) — `parseFormat` accepte silencieusement `?format=csv&format=xlsx`** [`client/api/_lib/reports/export-csv-handler.ts:parseFormat`] — `Array.isArray(raw)` extrait `raw[0]`, le 2e est silently ignoré. Fix : `if (Array.isArray(raw) && raw.length > 1) return null` → 400 INVALID_FILTERS.
- [x] [Review][Patch] **P4 (MEDIUM) — `RawMember.last_name`/`email` typés non-null mais DB peut renvoyer null** [`client/api/_lib/reports/export-csv-handler.ts:RawMember,memberFullName`] — TS-vs-DB mismatch : si `last_name` NULL (legacy data, anonymisation), `memberFullName` retourne `'Prénom null'` ou `String(null)='null'`. Fix : `last_name: string | null`, `email: string | null`, coalesce avec `?? ''` dans `memberFullName` + `projectExportRow`.
- [x] [Review][Patch] **P5 (MEDIUM) — `revokeObjectURL` sur microtask : Safari/FF peuvent annuler le download** [`client/src/features/back-office/composables/useSavExport.ts:triggerBrowserDownload`] — `Promise.resolve().then(() => revokeObjectURL(...))` revoke avant que Safari ait initié la requête de download → fichier vide silencieux, le composable retourne `'downloaded'`. Fix : `setTimeout(..., 30_000)` ou plus.
- [x] [Review][Patch] **P6 (MEDIUM) — `HAS_5_DIGITS` route toute requête FTS contenant 5 chiffres vers ilike (full-scan + résultats vides)** [`client/api/_lib/reports/export-csv-handler.ts:applyFilters,HAS_5_DIGITS`] — `q='entreprise12345'` ou `q='facture 678901'` → `ilike('reference', '%...%')` au lieu de `textSearch` ; user voit divergence vs SavListView + scan séquentiel. Fix : ancrer le regex `^\d{5,}$` ou exiger préfixe `^SAV-` ; sinon FTS.
- [x] [Review][Patch] **P7 (MEDIUM) — `'aborted'` magic-string contract entre composable et view** [`client/src/features/back-office/composables/useSavExport.ts` + `client/src/features/back-office/views/SavListView.vue:runExport`] — annulation utilisateur retourne `{status:'error', message:'aborted'}` ; la view filtre avec `result.message !== 'aborted'`. Refactor / i18n / changement message → toast erreur faux-positif. Fix : status discriminé `'aborted'` + view check `result.status === 'error'` exclusivement.
- [x] [Review][Patch] **P8 (MEDIUM) — `hashFilters` produit des hashs différents pour des filtres logiquement identiques** [`client/api/_lib/reports/export-csv-handler.ts:hashFilters`] — `JSON.stringify(filters, Object.keys(filters).sort())` utilise le 2e arg comme allow-list (pas comme key-sort) ; nested arrays (`status: ['closed','validated']` vs `['validated','closed']`) → digests différents → corrélation logs fragmentée. Fix : canonicaliser arrays imbriqués (`[...arr].sort()`) avant stringify ; éventuellement remplacer le hash maison par `crypto.createHash('sha256').digest('hex').slice(0,8)`.
- [x] [Review][Patch] **P9 (MEDIUM) — `downloading` reste à `true` si 2e appel `downloadExport` jette synchrone avant fetch** [`client/src/features/back-office/composables/useSavExport.ts:downloadExport`] — `controller = ac` réassigné ; si `buildExportQuery` ou autre throw sync avant le `try`, l'appel #1 toujours en vol voit `controller !== ac` dans son `finally` et ne reset pas `downloading.value`. Fix : compteur `inFlight` ou reset défensif `if (controller === ac || controller === null)`.
- [x] [Review][Patch] **P10 (MEDIUM) — Pas d'assert `rows.length === rowCount` après fetch (truncation silencieuse db-max-rows)** [`client/api/_lib/reports/export-csv-handler.ts:coreHandler`] — TOCTOU entre count et fetch + cap PostgREST `db-max-rows` non vérifié (W49) → export possiblement incomplet sans signal. Fix : log warn `export.csv.possibly_truncated` si `rows.length < rowCount` ou `rows.length === HARD_LIMIT_ROWS` ; éventuellement répondre `EXPORT_TRUNCATED` côté serveur.
- [x] [Review][Patch] **P11 (LOW) — `extractSuppliers` accepte whitespace-only `supplier_code`** [`client/api/_lib/reports/export-csv-handler.ts:extractSuppliers`] — `code.length === 0` ne capture pas `'  '` ou `'\t'` → cellule `'RUFINO |   | BIOSUD'`. Fix : `code.trim()` avant check + dedup.
- [x] [Review][Patch] **P12 (LOW) — `extractMotifs` peut dépasser le cap Excel 32 767 chars/cellule** [`client/api/_lib/reports/export-csv-handler.ts:extractMotifs`] — SAV avec 100 lignes × 300 chars motifs = 30k+ concaténés → XLSX corrompu, CSV illisible. Fix : truncate à 32 000 chars + suffix `…` ou cap à N motifs (e.g. 20).
- [x] [Review][Patch] **P13 (LOW) — Export 0 lignes → fichier header-only sans signal UI** [`client/api/_lib/reports/export-csv-handler.ts:coreHandler` + `client/src/features/back-office/views/SavListView.vue:runExport`] — toast "Export téléchargé." avec un fichier vide trompe l'opérateur. Fix : si `rowCount === 0`, retourner `200 JSON { warning: 'EMPTY_RESULT', row_count: 0 }` (pattern SWITCH_TO_XLSX) + UI toast info.
- [x] [Review][Patch] **P14 (LOW) — Toast success persiste indéfiniment** [`client/src/features/back-office/views/SavListView.vue`] — pas d'auto-dismiss → 5 exports successifs laissent le dernier toast accroché sans contexte temporel. Fix : `setTimeout(() => { if (exportToast.value?.variant === 'success') dismissToast() }, 4000)`.
- [x] [Review][Defer] W53 (Edge HIGH) — `count: 'exact'` HEAD scan peut OOM/timeout sur grosses tables [`client/api/_lib/reports/export-csv-handler.ts:countQ`] — deferred, optimisation scale event
- [x] [Review][Defer] W54 (Blind MEDIUM) — Aucun timeout (`AbortSignal.timeout`) sur les Supabase queries [`client/api/_lib/reports/export-csv-handler.ts`] — deferred, transverse à plusieurs handlers Epic 5
- [x] [Review][Defer] W55 (Blind LOW) — SheetJS `xlsx` historique CVE (proto-pollution, ReDoS) [`client/api/_lib/reports/xlsx-generator.ts`] — deferred, vérifier version pinned ≥ 0.20.2 ou migrer `exceljs` au prochain audit deps
